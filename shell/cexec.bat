...
echo Setting up vcvars...
    if "%TargetArch%"=="x64" (
        ...
        call "C:\Program Files (x86)\Microsoft Visual Studio\2017\Community\VC\Auxiliary\Build\vcvars32.bat" > NUL
    )
)
...
cl /nologo /W3 /WX /GR- /EHa /std:c++17 /Fo%ScriptIntermediatesDirectory%\ %1 /link /OUT:%ScriptIntermediatesDirectory%\executable.exe /PDB:%ScriptIntermediatesDirectory%\executable.pdb 1>%ScriptIntermediatesDirectory%\compiler-output.txt
